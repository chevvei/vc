...
**边界细节（面试加分点）**：
- cell 区间**左闭右开** `[ox+k·cs, ox+(k+1)·cs)`：落在原点 x=ox 的点属于格 0；但恰好压在包围盒**右边缘**的点 `cx == gx` → 也算越界。"范围外"不只指远处离群点，盒边上的点也是
- **floorf 不能换成 (int) 强转**：`(int)` 向零截断，floor 向下取整，负数时结果不同。越界点 x=-100.5 算出 -0.25：`floorf(-0.25) = -1` 被 `cx < 0` 正确抓住；`(int)(-0.25) = 0` 则**静默走私进 cell 0**——正是本 bug 的放大器。floorf 是越界检测的守护者
- 参数速查：ox/oy/oz=origin 包围盒原点（本项目 -100）；cs=cell size 格子边长；gx/gy/gz=每轴格数（200/cs）；`(x-ox)/cs` = 相对距离换算成"第几格"
