...
1. 为什么叫蝴蝶？举个小例子（4 个线程 lane 0,1,2,3）
   取 laneMask = 0b10（也就是 2）
   lane0：0 ^ 2 = 2 → 0 ↔ 2
   lane2：2 ^ 2 = 0 → 2 ↔ 0
   lane1：1 ^ 2 = 3 → 1 ↔ 3
   lane3：3 ^ 2 = 1 → 3 ↔ 1
   连线：0 连 2、1 连 3。
   画成图：两组交叉连线，左右对称，像一对蝴蝶翅膀，所以叫蝶形交换、蝴蝶寻址。
   特点：
   对称：A 找 B，B 也找 A（id ^ mask ^ mask = id），双向配对
   不重叠：每一层里，所有配对互不冲突
   分层：mask 依次取 16,8,4,2,1（2 的幂），一层一层缩小配对距离，刚好是 FFT 蝶形运算、warp 归约的经典log2N结构
